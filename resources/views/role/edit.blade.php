            ...
            <div class="container mt-4">
                ...
                <form method="POST" action="{{ route('roles.update', $role->id) }}">
                        ...
                        <div class="col-md-4">
                            <div class="card bg-light">
                                <h5 class="card-header bg-transparent border-bottom mt-0">
                                    <input type="checkbox" class="parent_main" id="{{$value->name}}_parent" onclick="check('{{$value->name}}')" {{ in_array($value->name, $rolePermissions) ? 'checked' : '' }}>
                                ...
                                <div class="card-body">
                                    <ul id="{{$value->name}}" class="test-sub-main">
                                        ...
                                        <li>
                                            <input type="checkbox" name="permission[{{ $key->name }}]" id="{{$value->name}}_{{$key->name}}" value="{{ $key->name }}" class='permission' {{ in_array($key->name, $rolePermissions) ? 'checked' : '' }}>
                    ...
                    <div class="form-footer mt-2">
                        ...
                        <a href="{{ route('roles.index') }}" class="btn btn-info" style="margin-right:5px">Back</a>
                    </div>
                </form>
            </div>
        </div>
