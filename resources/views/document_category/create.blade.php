...
@section('content')
    ...
    <div class="card mt-4">
        ...
        @endif
            <a href="{{route('document_category.index')}}" class="btn btn-primary">{{ __('home.all_categories') }}</a>

        </div>
        <div class="card-body ">
            <form action="{{isset($category) ? route('document_category.update', $category) : route('document_category.store')}}" method="POST" enctype="multipart/form-data">
                @csrf
                @if(isset($category))
                    @method('PUT')
                @else
                        ...
                        <div class="col-xl-2 px-3 px-xl-1">
                            ...
                            <div class="form-group">
                                ...
                                <label class="custom-switch form-switch mb-0">
                                    <input type="checkbox" name="active" class="custom-switch-input" @if(isset($category)) @if($category->status == 1) {{'checked'}} @endif @else {{ 'checked' }}@endif>
