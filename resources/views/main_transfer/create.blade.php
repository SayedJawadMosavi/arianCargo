...
@section('title', isset($main_transfer) ? 'Edit Main Transfer' : 'New Main Transfer')
@section('content')
    <div class="card mt-4">
        ...
        <div class="card-header d-flex justify-content-between">
            <h4 class="card-title">
                {{ isset($main_transfer) ? __('home.edit_main_transfer') : __('home.new_main_transfer') }}
            </h4>
                ...
                <div class="form-row mb-3">
                    @if ($settings->date_type == 'shamsi')
                        <div class="col-xl-3 mb-3">
                            ...
                            <input type="text" class="form-control @error('date') is-invalid @enderror" name="shamsi_date" autocomplete="off" id="dates" value="{{ isset($main_transfer) ? $main_transfer->shamsi_date : old('shamsi_date') }}">
                            @error('date')
                    ...
                    <div class="col-xl-4 mb-3">
                        ...
                        <select class="form-select form-control @error('stock_id') is-invalid @enderror" id="validationServer04" aria-describedby="validationServer04Feedback" required name="stock_id">
                            ...
                            @foreach($stocks as $stock)
                                <option value="{{ $stock->id }}" {{ isset($main_transfer) && $main_transfer->stock_id == $stock->id ? 'selected' : '' }}>{{ $stock->name }}</option>
                            @endforeach
                        </select>
                        @error('stock_id')
                            <div id="" class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-sm-12">
                        <div class="table-responsive">
                            ...
                            <table class="table table-bordered table-striped" id="user_table">
                                ...
                                <tbody id="tbody">
                                    @if(isset($main_transfer))
                                        @foreach($main_transfer->details as $detail)
                                            <tr>
                                                <td>
                                                    <select class="form-select form-control select2" name="product[]" onchange="loadAvailable(this, {{ $loop->index + 1 }})">
                                                        <option value="">{{ __('home.please_select') }}</option>
                                                        @foreach ($products as $obj)
                                                            <option value="{{ $obj->product_id }}" data-qty="{{ $obj->total_quantity }}" {{ $detail->product_id == $obj->product_id ? 'selected' : '' }}>
                                                                {{ $obj->product->name }} - {{ $obj->product->model }} ({{ $obj->total_quantity }})
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td><input type="number" name="available[]" readonly class="form-control" value="{{ $detail->available }}" /></td>
                                                <td><input type="number" name="quantity[]" class="form-control" value="{{ $detail->quantity }}" oninput="validateQuantity(this)" /></td>
                                                <td>
                                                    @if ($loop->index > 0)
                                                        <button type="button" name="remove" class="btn btn-danger remove"><i class="fa fa-minus"></i></button>
                                                    @else
                                                        <button type="button" name="add" id="add" class="btn btn-success btn-primary"><i class="fa fa-plus"></i></button>
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach
                                    @else
                                        <tr>
                                            <td>
                                                <select class="form-select form-control select2" name="product[]" onchange="loadAvailable(this, 1)">
                                                    <option value="">{{ __('home.please_select') }}</option>
                                                    @foreach ($products as $obj)
                                                        <option value="{{ $obj->product_id }}" data-qty="{{ $obj->total_quantity }}">
                                                            {{ $obj->product->name }} - {{ $obj->product->model }} ({{ $obj->total_quantity }})
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </td>
                                            <td><input type="number" name="available[]" readonly class="form-control" value="0" /></td>
                                            <td><input type="number" name="quantity[]" class="form-control" value="0" oninput="validateQuantity(this)" /></td>
                                            <td><button type="button" name="add" id="add" class="btn btn-success btn-primary"><i class="fa fa-plus"></i></button></td>
                                        </tr>
                                    @endif
                                </tbody>
                            </table>
...
<script>
    $(document).ready(function() {
        var count = {{ isset($main_transfer) ? $main_transfer->details->count() : 1 }};
