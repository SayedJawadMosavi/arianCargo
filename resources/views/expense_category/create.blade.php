...
@section('content')
    ...
    <div class="card mt-4">
        ...
        @endif
            <a href="{{route('expense_category.index')}}" class="btn btn-primary">{{ __('home.all_categories') }}</a>

        </div>
        <div class="card-body ">
            <form action="{{isset($category) ? route('expense_category.update', $category) : route('expense_category.store')}}" method="POST" enctype="multipart/form-data">
                @csrf
                @if(isset($category))
                    @method('PUT')
                @else
