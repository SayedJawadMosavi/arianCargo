@extends('layouts.app')
@section('title', 'Available Stock Report')
...
<div class="card mt-4">
    ...
    <div class="card-header ">
        <h3 class="card-title">{{ __('home.available_stock_report') }}</h3>
    </div>
    <div class="card-body pt-4">
        <div class="col-sm-12 my-3">
            <form action="{{ route('report.available_stock.post') }}" method="POST">
                ...
                <div class="form-row align-items-center my-5 offset-md-1">
                    <div class="col-xl-3">
                        ...
                        <select class="form-selects form-control" id="validationServer04" aria-describedby="validationServer04Feedback" required name="stock_id">
            ...
            <table id="file-datatable" class="table table-bordered table-striped text-nowrap mb-0 table-hover">
                ...
                <tbody>
                    ...
                    <td>{{$loop->iteration}}</td>
                        <td>{{$obj->subProduct->product->category->name}}</td>
                        <td>{{$obj->subProduct->product->name}}</td>
                        <td>{{$obj->subProduct->product->unit->name}}</td>
                        <td>{{$obj->stockProduct->stock->name}}</td>
                        <td>{{$obj->available}}</td>
