...
function datenow(){
    return x_date(date_to_persian(date("Y-m-d")));
}



//funtion to find this year
function ymd($key)
{
	return findymd(date_to_persian(date("Y-m-d")),$key);
}
...
function convert_date_to_persian($value){
	if($value){
		$splitedDate = explode( " ", mds_date("j F Y", strtotime($value), 1) );
		return $splitedDate[0]." ".$splitedDate[1].( $splitedDate[2] != mds_date("Y", time(), 1) ? " ".$splitedDate[2] : "" );
	}else
		return "---";
}
function date_to_persian($value, $format="j m Y"){
	if($value=='')
		$value = date('Y-m-d H:i:s');

	return mds_date($format, strtotime($value), 1);
}
