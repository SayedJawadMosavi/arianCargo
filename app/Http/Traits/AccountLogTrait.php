...
trait AccountLogTrait
{
    ...
    private function InsertAccountLog2($account, $type, $amount, $desc, $available, $action = 'direct', $action_id = null, $branch_id = null, $date)
    {
        $flag = false;
        is_null($available) ? $available = 0 : $available = $available;
        $settings = request()->attributes->get('settings');
        ...
        $log = AccountLog::create([
            ...
            'action_id' => $action_id,
            'user_id' => auth()->user()->id,
            'branch_id' => $branch_id ?? auth()->user()->branch_id, // Use passed branch_id or fallback
        ]);

        $flag = $settings->date_type == 'shamsi'
            ? $log->update(['shamsi_date' => $date])
            : $log->update(['miladi_date' => $date]);
